...
export const generateExam = action({
  args: {
    ...
    maxAttempts: v.optional(v.number()),
    instantFeedback: v.optional(v.boolean()),
    // Actual school grade (0 = Grade R, 1-12). Previously the prompt used
    // difficulty as the "grade level" which was wrong — difficulty is
    // Easy/Medium/Hard, not a grade. Grade drives curriculum appropriateness.
    grade: v.optional(v.number()),
    // South African exam type: "Topic Test", "Term Test", "June Exam",
    // "Preliminary Exam", "Final Exam (November)", "SBA Task", "Quiz"
    southAfricanExamType: v.optional(v.string()),
  },
  handler: async (ctx, args) => {
    ...
    // ─── GRADE-AWARE CONTEXT ─────────────────────────────────────
    // The grade (0=Grade R … 12) drives the curriculum level. When the
    // frontend can't send it, we derive it from the class name ("Grade 10A").
    ...
    if (grade === undefined || grade < 0 || grade > 12) {
      ...
      const parsed = classObj?.name ? parseInt(String(classObj.name).replace(/\D/g, ""), 10) : NaN;
      grade = Number.isNaN(parsed) ? undefined : parsed;
    }
    ...
    // CAPS-aligned cognitive demand by phase (Bloom's taxonomy)
    const COGNITIVE_BY_PHASE: Record<string, string> = {
      ...
      "FET Phase (Grades 10-12)": "full range of cognitive levels: knowledge, comprehension, application, analysis, evaluation, synthesis — matching NSC examination verb conventions (state, define, describe, explain, discuss, evaluate, compare, analyse)",
    };

    // Grade-appropriate language register
    ...
    // SA exam-type guidance (duration, mark totals, structure conventions)
    const SA_EXAM_TYPE_GUIDANCE: Record<string, string> = {
      "Topic Test": "Short assessment covering ONLY the selected topics. Foundation/Intermediate: 15-25 marks. Senior: 25-30 marks. FET: 30-50 marks. Duration 20-45 min.",
      "Term Test": "End-of-term assessment covering the term's work. Foundation: 25-40 marks. Intermediate: 40-50 marks. Senior: 50-60 marks. FET: 50-100 marks. Duration 45-90 min.",
      "June Exam": "Mid-year examination covering terms 1-2. Foundation: 40-50 marks. Intermediate: 60-75 marks. Senior: 80-100 marks. FET: 100-150 marks (or per subject paper convention, e.g. Maths Paper 1). Duration 60-180 min.",
      "Preliminary Exam": "Full mock NSC/full-year exam. FET only: 150 marks, matching final paper conventions. Duration 150-180 min.",
      "Final Exam (November)": "Full-year summative examination. Foundation: 50-60 marks. Intermediate: 75-100 marks. Senior: 100 marks. FET: 150-300 marks depending on subject and paper count. Duration 90-180 min.",
      "SBA Task": "School-Based Assessment task (project, investigation, orals, practicals, research). Include a clear task statement with criteria. Marks per CAPS SBA requirements (typically 50 marks).",
      "Quiz": "Informal practice assessment with instant feedback; 5-15 questions; 1-3 marks each.",
    };

    // Question difficulty calibration per grade (NOT the same as the UI "difficulty" filter)
    const GRADE_CALIBRATION = (g: number | undefined): string => {
      if (g === undefined) return args.difficulty;
      const base = args.difficulty === "Easy" ? "easy" : args.difficulty === "Hard" ? "hard" : "medium";
      if (g <= 3) {
        return base === "hard" ? "medium (appropriate for the grade)" : "easy-to-medium (concrete, age-appropriate)";
      }
      if (g <= 6) return `${base}-to-${base === "easy" ? "medium" : "hard"} with age-appropriate scaffolding`;
      if (g <= 9) return `${base} with clear instruction verbs and scaffolding on hard questions`;
      return `${base} at full NSC standard (hard questions should use NSC-style verb conventions)`;
    };

    const phase = grade !== undefined ? PHASE_FOR_GRADE(grade) : "Foundation Phase (Grades 1-3)";
    const cognitive = COGNITIVE_BY_PHASE[phase];
    const language = LANGUAGE_BY_PHASE[phase];
    const calibration = GRADE_CALIBRATION(grade);
    const saExamGuidance =
      SA_EXAM_TYPE_GUIDANCE[args.southAfricanExamType || ""] ||
      SA_EXAM_TYPE_GUIDANCE[examType === "quiz" ? "Quiz" : "Term Test"];

    const gradeLabel = grade === undefined ? "unknown (infer from question difficulty)" : grade === 0 ? "Grade R (Pre-school)" : `Grade ${grade}`;
  ...
  "difficulty": "DIFFICULTY"
    ...
    const prompt = `You are an expert South African CAPS curriculum teacher creating a ${examType === "quiz" ? "practice quiz" : "formal exam"} for a home schooling centre.

CONTEXT:
- Subject: ${subjectName} (${subjectCategory})
- Topics: ${topicsList}
- School Grade: ${gradeLabel}
- CAPS Phase: ${phase}
- Assessment Type: ${args.southAfricanExamType || (examType === "quiz" ? "Quiz" : "Term Test")}
- ${saExamGuidance}
- Cognitive levels expected: ${cognitive}
- Language register: ${language}
- Question difficulty calibration: ${calibration}
...
11. Questions MUST be written for ${gradeLabel} (${phase}): use the expected cognitive levels and language register described above. Never write questions above or below the grade level. A Grade 3 question must be answerable by a 9-year-old; a Grade 12 question must meet full NSC standard.
12. Total marks should fit the assessment type (${saExamGuidance}).
13. Use South African context, names, currency (rand), geography and examples where relevant.`;
