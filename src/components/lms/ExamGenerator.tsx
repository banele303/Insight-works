...
const schema = z.object({
  ...
  southAfricanExamType: z.string().optional(),
...
// South African assessment types (CAPS-aligned)
const SA_EXAM_TYPES = [
  "Topic Test",
  "Term Test",
  "June Exam",
...
// CAPS phase from grade
const PHASE_FOR_GRADE = (g: number | undefined): string => {
  if (g === undefined) return "Unknown phase";
...
const ExamGenerator = ({ open, onOpenChange, onSuccess }: Props) => {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      examType: "exam",
      southAfricanExamType: "Term Test",
      topics: [],
      duration: 60,
      difficulty: "Medium",
      questionMix: [
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      // Full shape — multi-topic + question type mix + grade-aware SA exam type
      const fullPayload = {
        ...legacyPayload,
        ...
        instantFeedback: values.instantFeedback,
        grade: classGrade,
        southAfricanExamType: values.southAfricanExamType,
      };
        ...
        {step === "template" ? (
          ...
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <FieldGroup className="space-y-5">
              ...
              {/* Grade + Phase indicator */}
              {selectedClass && (
                <div className="flex flex-wrap gap-2">
                  <Badge className="bg-emerald-500/15 text-emerald-500 border-emerald-500/30" variant="outline">
                    {classGrade === 0 ? "Grade R" : classGrade ? `Grade ${classGrade}` : "Grade unknown"} • {PHASE_FOR_GRADE(classGrade)}
                  </Badge>
                  <Badge variant="outline" className="text-muted-foreground">
                    Questions will be written for this grade level
                  </Badge>
                </div>
              )}
              ...
              {/* SA Assessment Type */}
              <Controller
                name="southAfricanExamType"
                control={form.control}
                render={({ field }) => (
                  <Field>
                    <FieldLabel className="flex items-center gap-2">
                      <GraduationCap className="h-4 w-4" />
                      Assessment Type (CAPS)
                    </FieldLabel>
                    <Select
                      onValueChange={(v) => {
                        field.onChange(v);
                        // Auto-tune duration for common SA assessment types
                        const DUR: Record<string, number> = {
                          "Topic Test": 30,
                          "Term Test": 60,
                          "June Exam": 120,
                          "Preliminary Exam": 180,
                          "Final Exam (November)": 180,
                          "SBA Task": 45,
                          Quiz: 10,
                        };
                        if (DUR[v]) form.setValue("duration", DUR[v]);
                      }}
                      value={field.value}
                    >
                      <SelectTrigger>
                    ...
                    <p className="text-xs text-muted-foreground mt-1.5">
                      The exam type sets the mark total, structure and cognitive levels to match CAPS conventions for the selected grade.
                    </p>
                  </Field>
                )}
              />
