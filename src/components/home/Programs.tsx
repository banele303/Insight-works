      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          ...
          <p className="text-gray-400 max-w-md">
            Glenanda Shopping Learning Center supports all CAPS learning areas from Grade R to Grade 12 — designed
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {subjects.map((subject, idx) => (
            <div
              key={idx}
              className="group relative bg-gray-50 dark:bg-[#1c1c1c] border border-white/[0.08] p-8 rounded-2xl hover:border-[#f97316]/50 transition-all duration-300 shadow-sm hover:shadow-xl"
            >
              ...
              <div className="bg-white/[0.03] w-14 h-14 rounded-xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform shadow-sm border border-gray-100 dark:border-gray-700">
                <subject.icon className="text-[#f97316] w-7 h-7" />
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {subject.tags.map((tag, tIdx) => (
                  <span
                    key={tIdx}
                    className="px-3 py-1 bg-white/[0.03] border border-white/[0.08] rounded-full text-xs font-medium text-gray-500 dark:text-gray-400"
                  >
                    {tag}
                  </span>
                ))}
              ...
              <button className="mt-8 flex items-center text-[#f97316] font-bold group-hover:translate-x-2 transition-transform">
